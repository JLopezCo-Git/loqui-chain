...
- **arqueos_caja / arqueo_items**: conteo físico de caja (ver `Cuentas` en `Cadena2026.xlsx`) — no es lo mismo que `caja_movimientos` (que es teórico, según lo registrado). `arqueoRoutes.js` calcula `esperado` en el servidor (`SUM(valor_pagado)` de las obligaciones de esa cadena, nunca a mano ni confiando en el cliente) y cada arqueo guarda `efectivo_contado` (con el desglose de billetes en `denominaciones`, JSON) más una lista libre de `arqueo_items` (etiqueta + monto — cuentas digitales, préstamos, gastos por discriminar; no son dos casillas fijas). `faltaReponer = esperado - efectivo_contado - SUM(items.monto)` se calcula al leer, nunca se guarda (evita que quede desactualizado). Cada guardado es una foto nueva (historial), no se edita un arqueo existente.
...
- `CadenaGrid.tsx` (desktop) tiene: rango de quincenas visibles (`rango: [desde,hasta]` sobre índices de `data.quincenas`, sin refetch — es slicing local), toggle "Por fecha" (matriz) / "Por participante" (lista con acordeón por fila), filtro "Solo pendientes" (oculta filas sin ninguna celda pendiente/vencida/parcial en el rango visible), barra de progreso por fila, fracción de pagos recibidos por columna (`X/Y`) junto al total en dinero, y exportar CSV (client-side, `Blob` + link temporal, sin endpoint nuevo).
